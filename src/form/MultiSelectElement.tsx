...
export type MultiSelectElementProps = SelectProps & Props
...
const MultiSelectElement: React.FunctionComponent<MultiSelectElementProps> = ({
  ...
  return (
    <FormControl style={{ minWidth }} fullWidth={rest.fullWidth} error={!!errorMessages}>
      <InputLabel htmlFor="select-multiple" required={required}>{label}</InputLabel>
      <Select
        multiple
        value={formValue || []}
        required={required}
      ...
      >
        {menuItems.map((item: any) => (
          <MenuItem
            ...
            style={{ fontWeight: (formValue || []).includes(item) ? 'bold' : 'normal' }}>
